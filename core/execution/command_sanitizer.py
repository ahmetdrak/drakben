...
class CommandSanitizer:
    ...
    ALLOWED_BINARIES: ClassVar[frozenset[str]] = frozenset({
        ...
        "whatweb", "wafw00f", "httpx", "httprobe",
        # ── Web Application ──
        "nikto", "dirb", "dirbuster", "gobuster", "feroxbuster", "ffuf",
        "wfuzz", "sqlmap", "commix", "xsstrike",
        "nuclei", "dalfox", "arjun", "paramspider",
        "burpsuite", "zap", "zaproxy",
        # ── Exploitation ──
        "msfconsole", "msfvenom", "searchsploit",
        "hydra", "medusa", "john", "hashcat",
        "responder", "impacket-smbexec", "impacket-wmiexec",
        "impacket-psexec", "crackmapexec", "netexec",
        "evil-winrm", "bloodhound", "sharphound",
        # ── Network / Transport ──
        "curl", "wget", "nc", "ncat", "netcat", "socat",
        "ssh", "scp", "sftp", "telnet", "ftp",
        "tcpdump", "tshark", "wireshark",
        "traceroute", "tracert", "ping", "arping", "hping3",
        "arp-scan", "ettercap", "bettercap", "mitmproxy",
        # ── Wireless ──
        "aircrack-ng", "airmon-ng", "airodump-ng", "aireplay-ng",
        "wifite", "kismet", "reaver",
        # ── Post-Exploitation ──
        "mimikatz", "lazagne", "seatbelt", "winpeas", "linpeas",
        "pspy", "chisel", "ligolo",
        # ── General utilities (safe) ──
        "cat", "head", "tail", "less", "more", "grep", "awk",
        "sed", "cut", "sort", "uniq", "wc", "tr", "find",
        "ls", "dir", "echo", "printf", "date", "id", "whoami",
        "uname", "hostname", "ifconfig", "ip", "netstat", "ss",
        "ps", "top", "file", "strings", "xxd", "hexdump",
        "base64", "openssl", "certutil",
        "python", "python3", "pip", "pip3",
        "ruby", "perl", "php", "node",
        "git", "svn",
        "docker", "kubectl",
        "systemctl", "service", "journalctl",
        "chmod", "chown", "mkdir", "cp", "mv", "touch",
        "tar", "gzip", "gunzip", "zip", "unzip",
        "mount", "umount", "df", "du", "free",
        # ── Windows equivalents ──
        "type", "where", "findstr", "icacls", "net", "sc",
        "wmic", "tasklist", "ipconfig", "nbtstat", "arp",
        "route", "pathping", "certutil.exe", "powershell",
        "cmd", "cmd.exe", "reg",
    })
    ...
    def is_allowed_binary(cls, command: str) -> bool:
        ...
        return (
            binary in cls.ALLOWED_BINARIES
            or binary.removesuffix(".exe") in cls.ALLOWED_BINARIES
        )
