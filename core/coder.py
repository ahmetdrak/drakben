...
class ASTSecurityChecker(ast.NodeVisitor):
    ...
    DANGEROUS_FUNCTIONS = {
        ...
        'getattr', 'setattr', 'delattr',  # Can be used for attribute manipulation
                ...
                if len(parts) >= 2:
                    ...
                    if (module_name, func) in self.RESTRICTED_CALLS:
                        ...
                        if module_name == 'subprocess' and self.allow_subprocess_tools:
                            if self._is_allowed_subprocess_call(node):
                                pass  # Allowed
                            else:
                                self.violations.append(
...
class AICoder:
    ...
    def __init__(self, brain):
        ...
        init_file = DYNAMIC_MODULES_PATH / "__init__.py"
...
Output ONLY the Python code, no explanations."""
        ...
        user_msg = f"""The tool '{failed_tool}' failed with error: {error_message}
...
"""
        ...
        try:
            ...
            if not self.brain or not self.brain.llm_client:
                logger.warning("No LLM client available for tool creation")
...
"""
    ...
    def _security_check(self, code: str) -> bool:
        ...
        """
        result = self._security_check_ast(code)
        return result["safe"]
