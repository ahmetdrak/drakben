...
class Policy:
    ...
    policy_id: str
    # {"target_type": "web", "error_type": "timeout"}
    condition: Dict[str, Any]
    # {"avoid_tools": ["sqlmap"], "prefer_strategy": "stealth"}
    action: Dict[str, Any]
    weight: float
...
class SelfRefiningEngine:
    ...
    # Thresholds
    PROFILE_RETIRE_THRESHOLD = 0.15  # Retire if success_rate < 15% (More lenient)
    MIN_USAGE_FOR_RETIRE = 5         # Minimum uses before retirement (More chances)
            ...
            try:
                logger.info(
                    f"Initializing SelfRefiningEngine database (attempt {
                        self._init_attempts}/3)...")
                self._init_database()
                self._seed_default_strategies()
                self._initialized = True
                logger.info(
                    "SelfRefiningEngine database initialized successfully")
    ...
    def _db_operation(self, timeout: float = 5.0):
        ...
        if not acquired:
            raise TimeoutError(
                "Database lock acquisition timeout - possible deadlock")
    ...
    def _acquire_db_lock(
            self,
            max_retries: int = 3,
            timeout_per_attempt: float = 1.5) -> bool:
        """
    ...
    def _create_initial_profiles(
        ...
        profile2 = {
            ...
            # Different order
            "step_order": ["scan", "exploit", "recon", "analyze"],
            "aggressiveness": 0.8,
            "tool_preferences": []
        }
        ...
        for key in conservative_params:
            if isinstance(conservative_params[key], (int, float)):
                conservative_params[key] = max(
                    1, int(conservative_params[key] * 0.5))
        ...
        profile3 = {
            ...
            # Different order
            "step_order": ["recon", "analyze", "scan", "exploit"],
            "aggressiveness": 0.2,
            "tool_preferences": []
        }
    ...
    def get_strategies_for_target_type(
            self, target_type: str) -> List[Strategy]:
    ...
    def get_profiles_for_strategy(
        ...
        with self._lock:
            ...
            try:
                if include_retired:
                    cursor = conn.execute(
                        "SELECT * FROM strategy_profiles WHERE strategy_name = ?", (strategy_name,))
                else:
                    cursor = conn.execute(
                    ...
                    profiles.append(
                        StrategyProfile(
                            ...
                            parameters=json.loads(
                                row["parameters"]),
                            step_order=json.loads(
                                row["step_order"]),
                            aggressiveness=row["aggressiveness"],
                            tool_preferences=json.loads(
                                row["tool_preferences"]) if row["tool_preferences"] else [],
    ...
    def _apply_mutation(self, params: Dict, steps: List[str],
        ...
        for key in new_params:
            if isinstance(new_params[key], (int, float)):
                # Change by ±20%
                change = random.uniform(-self.MUTATION_PARAM_CHANGE,
                                        self.MUTATION_PARAM_CHANGE)
    ...
    def update_profile_outcome(
            self,
            profile_id: str,
            success: bool) -> Optional[StrategyProfile]:
        """
        ...
        with self._lock:
            ...
            try:
                ...
                if row["error_type"] in ["blocked", "banned", "firewall"]:
                    ...
                    weight = 0.7
                elif row["error_type"] == "network_error":
                    # Network error -> Dont ban profile, just slow down or retry
                    action = {"retry_delay_multiplier": 2.0}
    ...
    def get_failed_profiles_for_target(
            self, target_signature: str) -> List[str]:
    ...
    def select_strategy_and_profile(self,
                                    target: str) -> Tuple[Optional[Strategy],
        ...
        if not self._acquire_lock_safe(max_retries=3):
            logger.error(
                "Failed to acquire lock within timeout - possible deadlock")
            return None, None
        ...
        try:
            ...
            strategy = self._select_strategy(
                target_info, start_time, max_duration)
            if not strategy:
                return None, None
    ...
    def _select_strategy(self, context, start_time, max_duration):
        ...
        # Get strategies
        strategies = self.get_strategies_for_target_type(
            context["target_type"])
        if not strategies:
            return None
    ...
    def _select_profile(self, strategy, context, start_time, max_duration):
        ...
        profiles = self.get_profiles_for_strategy(
            strategy.name, include_retired=False)
    ...
    def get_evolution_status(self) -> Dict:
        ...
        with self._lock:
            ...
            try:
                ...
                """)
                status["policies_by_tier"] = {PolicyTier(
                    row[0]).name: row[1] for row in cursor.fetchall()}
