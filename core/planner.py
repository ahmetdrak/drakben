...
class Planner:
    ...
    def replan(self, failed_step_id: str) -> bool:
        ...
        if self._total_replans >= self.MAX_REPLAN_PER_SESSION:
            ...
            return self._skip_step(
                step, f"Session replan limit exceeded ({
                    self._total_replans}x)")
    ...
    def _apply_adaptive_learning(self, step: PlanStep, context: Dict):
        ...
        if context["is_timeout"]:
            ...
            self.memory.update_heuristic(
                "default_timeout", lambda x: min(
                    x * 1.5, 300))
            self.memory.update_heuristic(
                "aggressiveness", lambda x: max(
                    x - 0.2, 0.1))
    ...
    def _try_switch_tool(self, step: PlanStep, context: Dict) -> bool:
        ...
        step.error = f"{reason}Switched {
            context['original_tool']} -> {alternative}"
    ...
    def _penalize_tool(self):
        ...
        self.memory.set_heuristic(
            "penalty_increment", min(
                20.0, current + 1.0))
    ...
    def _generate_steps_from_profile(
        ...
        phase_to_actions = {
            "recon": ["passive_recon"] if aggressiveness < 0.5 else [
                "port_scan",
                "service_scan"],
            "scan": ["web_vuln_scan"] if aggressiveness < 0.3 else [
                "vuln_scan",
                "web_vuln_scan"],
            "analyze": [
                "manual_review",
    ...
    def create_plan_for_target(
        ...
        This uses a TEMPLATE structure for better maintainability.
        """
        plan_id = f"plan_{uuid.uuid4().hex[:8]}"

        # Base template (can be externalized later)
        plan_template = [
            {"action": "port_scan", "tool": "nmap_port_scan", "outcome": "discover_open_ports"},
            {"action": "service_scan", "tool": "nmap_service_scan", "outcome": "identify_services"},
            {"action": "vuln_scan", "tool": "nmap_vuln_scan", "outcome": "find_vulnerabilities"},
            {"action": "exploit", "tool": "sqlmap_scan", "outcome": "exploit_vulnerability"}
        ]

        steps = []
        for i, tmpl in enumerate(plan_template):
            steps.append({
                ...
                "status": StepStatus.PENDING.value,
                "max_retries": 1 if tmpl["action"] == "exploit" else 2,
                "retry_count": 0,
    ...
    # FIX: Removed duplicate replan() method - using the one defined at line
    # 147
    ...
    def get_plan_status(self) -> Dict:
        ...
        return {
            ...
            "total_steps": len(
                self.steps),
            "completed": sum(
                1 for s in self.steps if s.status == StepStatus.SUCCESS),
            "failed": sum(
                1 for s in self.steps if s.status == StepStatus.FAILED),
            "skipped": sum(
                1 for s in self.steps if s.status == StepStatus.SKIPPED),
            "pending": sum(
                1 for s in self.steps if s.status == StepStatus.PENDING),
