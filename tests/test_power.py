...
def test_power_level():
    ...
    for name, raw in payloads:
        obfuscated = waf.obfuscate_sql(raw) if name == "SQLi" else \
                     waf.obfuscate_xss(raw) if name == "XSS" else \
                     waf.obfuscate_shell(raw)
    ...
    if CURL_AVAILABLE:
        try:
            ...
            console.print(f"  [cyan]Impersonation:[/cyan] Chrome 120")
            console.print(f"  [cyan]User-Agent:[/cyan] {ua}")
            
            if "Chrome/120" in ua:
                console.print("  [bold green]✅ SUCCESS: User-Agent matches target fingerprint.[/bold green]")
            else:
                 console.print("  [bold red]❌ FAILED: User-Agent mismatch![/bold red]")
                 
            # Check Cipher Suite capability (Mock)
            console.print("  [bold green]✅ SUCCESS: curl_cffi core loaded successfully.[/bold green]")
            
        except Exception as e:
            console.print(f"  [bold red]❌ CRITICAL ERROR: {e}[/bold red]")
    else:
        console.print("  [bold red]❌ CRITICAL: curl_cffi not installed! Stealth module is offline.[/bold red]")
