...
def log_tool_execution(logger: logging.Logger, tool_name: str, target: str, success: bool, duration: float = 0.0):
    """Log tool execution with consistent format"""
    status = "SUCCESS" if success else "FAILED"
    logger.info(f"Tool: {tool_name} | Target: {target} | Status: {status} | Duration: {duration:.2f}s")
...
def log_vulnerability_found(logger: logging.Logger, vuln_id: str, service: str, port: int, severity: str):
    """Log vulnerability discovery"""
    logger.warning(f"Vulnerability found: {vuln_id} | Service: {service}:{port} | Severity: {severity}")
