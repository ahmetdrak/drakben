...
class SubdomainEnumerator:
    ...
    COMMON_SUBDOMAINS = [
        ...
        "webdisk", "pop3", "cpanel", "whm", "autodiscover", "autoconfig", "m", "imap",
        "test", "ns", "blog", "dev", "www2", "admin", "forum", "news", "vpn", "ns3",
        ...
        "app", "live", "owa", "en", "start", "sms", "office", "exchange", "ipv4", "git",
        "stage", "uat", "prod", "production", "sandbox", "jenkins", "gitlab", "jira",
        "confluence", "nagios", "zabbix", "grafana", "prometheus", "kibana", "elastic"
    ]
    ...
    def __init__(
        ...
        logger.info(
            f"Subdomain Enumerator initialized (subfinder: {
                self.subfinder_available}, amass: {
                self.amass_available})")
    ...
    def _build_enumeration_tasks(
            self,
            domain: str,
            use_bruteforce: bool) -> List:
    ...
    async def _process_enumeration_results(
            self,
            source_results: List,
            resolve: bool) -> List[SubdomainResult]:
        ...
        for result in source_results:
            if isinstance(result, list):
                results.extend(
                    self._extract_unique_subdomains(
                        result, subdomains))
    ...
    def _extract_unique_subdomains(
            ...
            subdomains: Set[str]) -> List[SubdomainResult]:
    ...
    def _extract_subdomains_from_name(
            self, name: str, domain: str) -> List[str]:
    ...
    async def _process_bruteforce_batches(
            self, domain: str, resolver) -> List[SubdomainResult]:
    ...
    async def _check_batch_subdomains(
            ...
            batch: List[str],
            domain: str,
            resolver) -> List[SubdomainResult]:
        """Check a batch of subdomains"""
        tasks = [
            self._check_single_subdomain(
                sub,
                domain,
                resolver) for sub in batch]
    ...
    async def _check_single_subdomain(
            ...
            sub: str,
            domain: str,
            resolver) -> Optional[SubdomainResult]:
    ...
    async def _resolve_in_batches(self,
                                  results: List[SubdomainResult],
                                  resolver) -> List[SubdomainResult]:
        ...
        for i in range(0, len(results), batch_size):
            ...
            tasks = [
                self._resolve_single_subdomain(
                    r, resolver) for r in batch]
            batch_results = await asyncio.gather(*tasks)
    ...
    async def _resolve_cname_record(
            self,
            result: SubdomainResult,
            resolver) -> None:
